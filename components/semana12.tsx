...
import {ejemplo_12} from "@/data/semanas"

export function Semana12Examples() {
    const [selectedExample, setSelectedExample] = useState<number | null>(null)

    return (
        <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
                {ejemplo_12.map((ejemplo) => (
                    <Card
                        key={ejemplo.id}
                            ...
                            <div className="flex items-start justify-between">
                                <div>
                                    <CardTitle className="text-lg flex items-center gap-2">
                                        {ejemplo.id === 1 && <Globe className="h-5 w-5 text-red-500"/>}
                                        {ejemplo.id === 2 && <FileText className="h-5 w-5 text-orange-500"/>}
                                        {ejemplo.titulo}
                                    </CardTitle>
                ...
                <Card className="mt-6">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            ...
                            {ejemplo_12.find((e) => e.id === selectedExample)?.titulo}
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <Tabs defaultValue="reflexion" className="w-full">
                            ...
                            <TabsContent value="reflexion" className="mt-4">
                                <div className="prose prose-sm max-w-none">
                                    <p>{ejemplo_12.find((e) => e.id === selectedExample)?.reflexion}</p>
                                </div>
                            </TabsContent>

                            <TabsContent value="codigo" className="mt-4">
                                <pre className="bg-muted p-4 rounded-lg overflow-x-auto text-sm">
                                    <code>{ejemplo_12.find((e) => e.id === selectedExample)?.codigo}</code>
                                </pre>
                            </TabsContent>
                        </Tabs>
                    </CardContent>
