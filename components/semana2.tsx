...
import {ejemplo_2} from "@/data/semanas";


export function Semana2Examples() {
    const [selectedExample, setSelectedExample] = useState<number | null>(null)

    return (
        <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
                {ejemplo_2.map((ejemplo) => (
                    <Card
                        key={ejemplo.id}
                ...
                <Card className="mt-6">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            ...
                            {ejemplo_2.find((e) => e.id === selectedExample)?.titulo}
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <Tabs defaultValue="reflexion" className="w-full">
                            ...
                            <TabsContent value="reflexion" className="mt-4">
                                <div className="prose prose-sm max-w-none">
                                    <p>{ejemplo_2.find((e) => e.id === selectedExample)?.reflexion}</p>
                                </div>
                            </TabsContent>

                            <TabsContent value="codigo" className="mt-4">
                <pre className="bg-muted p-4 rounded-lg overflow-x-auto text-sm">
                  <code>{ejemplo_2.find((e) => e.id === selectedExample)?.codigo}</code>
                </pre>
                            </TabsContent>
                        </Tabs>
                    </CardContent>
