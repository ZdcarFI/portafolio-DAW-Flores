...
import {ejemplo_5} from "@/data/semanas"


export function Semana5Examples() {
    const [selectedExample, setSelectedExample] = useState<number | null>(null)

    return (
        <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
                {ejemplo_5.map((ejemplo) => (
                    <Card
                        key={ejemplo.id}
            ...
            {selectedExample && (
                <Card className="mt-6">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            ...
                            {ejemplo_5.find((e) => e.id === selectedExample)?.titulo}
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <pre className="bg-muted p-4 rounded-lg overflow-x-auto text-sm">
                  <code>{ejemplo_5.find((e) => e.id === selectedExample)?.codigo}</code>
                </pre>
                            </TabsContent>
                        </Tabs>
                    </CardContent>
