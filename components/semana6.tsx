...
import {ejemplo_6} from "@/data/semanas"


export function Semana6Examples() {
    const [selectedExample, setSelectedExample] = useState<number | null>(null)

    return (
        <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
                {ejemplo_6.map((ejemplo) => (
                    <Card
                        key={ejemplo.id}
            ...
            {selectedExample && (
                <Card className="mt-6">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            ...
                            {ejemplo_6.find((e) => e.id === selectedExample)?.titulo}
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <pre className="bg-muted p-4 rounded-lg overflow-x-auto text-sm">
                  <code>{ejemplo_6.find((e) => e.id === selectedExample)?.codigo}</code>
                </pre>
                            </TabsContent>
