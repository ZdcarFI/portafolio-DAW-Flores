...
import {Target, Server, Code} from "lucide-react"
import {ejemplo_9} from "@/data/semanas"

export function Semana9Examples() {
    const [selectedExample, setSelectedExample] = useState<number | null>(null)

    return (
        <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
                {ejemplo_9.map((ejemplo) => (
                    <Card
                        key={ejemplo.id}
                            ...
                            <div className="flex items-start justify-between">
                                <div>
                                    <CardTitle className="text-lg flex items-center gap-2">
                                        {ejemplo.id === 1 && <Server className="h-5 w-5 text-green-500"/>}
                                        {ejemplo.id === 2 && <Code className="h-5 w-5 text-teal-500"/>}
                                        {ejemplo.titulo}
                                    </CardTitle>
                ...
                <Card className="mt-6">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            ...
                            {ejemplo_9.find((e) => e.id === selectedExample)?.titulo}
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <Tabs defaultValue="reflexion" className="w-full">
                            ...
                            <TabsContent value="reflexion" className="mt-4">
                                <div className="prose prose-sm max-w-none">
                                    <p>{ejemplo_9.find((e) => e.id === selectedExample)?.reflexion}</p>
                                </div>
                            </TabsContent>

                            <TabsContent value="codigo" className="mt-4">
                                <pre className="bg-muted p-4 rounded-lg overflow-x-auto text-sm">
                                    <code>{ejemplo_9.find((e) => e.id === selectedExample)?.codigo}</code>
                                </pre>
                            </TabsContent>
                        </Tabs>
                    </CardContent>
